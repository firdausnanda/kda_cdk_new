...
export default function Edit({ auth, data: transactionData, commodities }) {
  const { data, setData, put, processing, errors } = useForm({
    year: transactionData.year || new Date().getFullYear(),
    month: transactionData.month || new Date().getMonth() + 1,
    province_id: transactionData.province_id || 35,
    regency_id: transactionData.regency_id || '',
    district_id: transactionData.district_id || '',
    village_id: transactionData.village_id || '',
    nama_kth: transactionData.nama_kth || '',
    details: transactionData.details.map(d => ({
      id: d.id,
      commodity_id: d.commodity_id,
      volume_produksi: d.volume_produksi,
      satuan: d.satuan,
      nilai_transaksi: d.nilai_transaksi
    })) || [
  ...
  const submit = (e) => {
    e.preventDefault();
    put(route('nilai-transaksi-ekonomi.update', transactionData.id));
  };
  ...
  return (
    <AuthenticatedLayout user={auth.user} header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Edit Nilai Transaksi Ekonomi</h2>}>
      <Head title="Edit Nilai Transaksi Ekonomi" />
        ...
        <form onSubmit={submit} className="space-y-8">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-8 border-b border-gray-100 bg-gray-50/50">
              <h3 className="text-xl font-bold text-gray-900">Formulir Edit Data Transaksi</h3>
              <p className="mt-1 text-sm text-gray-500">Ubah rincian KTH dan lokasi transaksi.</p>
            </div>
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              <div className="md:col-span-2">
                <InputLabel htmlFor="nama_kth" value="Nama KTH" className="text-gray-700 font-bold mb-2" />
                <TextInput id="nama_kth" className="w-full" value={data.nama_kth} onChange={(e) => setData('nama_kth', e.target.value)} required />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-8 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">Ubah daftar komoditas dan nilai transaksinya.</p>
              </div>
      ...
      {/* Modal for Adding Commodity Same as Create */}
