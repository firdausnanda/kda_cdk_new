...
export default function BukanKayuIndex({ auth, bukanKayu, filters }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (modalMode === 'create') {
      post(route('bukan-kayu.store'), {
        onSuccess: () => closeModal(),
      });
    } else {
      put(route('bukan-kayu.update', currentData.id), {
        onSuccess: () => closeModal(),
      });
    }
  };
  ...
  const handleDelete = () => {
    destroy(route('bukan-kayu.destroy', currentData.id), {
  ...
  return (
    <AuthenticatedLayout
      user={auth.user}
      header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Master Data Bukan Kayu</h2>}
    >
      <Head title="Master Data Bukan Kayu" />
          ...
          <div className="bg-white overflow-hidden shadow-xl sm:rounded-2xl ring-1 ring-gray-900/5">
            ...
            <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center bg-gray-50/50">
              <h3 className="text-lg font-bold text-gray-900 mb-4 sm:mb-0">
                Daftar Bukan Kayu
                <p className="text-sm font-normal text-gray-500 mt-1">Kelola data jenis bukan kayu dalam sistem.</p>
              </h3>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {bukanKayu.data.length > 0 ? (
                    bukanKayu.data.map((item) => (
                    ...
                    <tr>
                      <td colSpan="3" className="px-6 py-12 text-center text-sm text-gray-500">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-500 text-lg font-medium">Tidak ada data Bukan Kayu</p>
            ...
            <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 rounded-b-2xl">
              <Pagination links={bukanKayu.links} />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-gray-900">
              {modalMode === 'create' ? 'Tambah Bukan Kayu' : 'Edit Bukan Kayu'}
            </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <InputLabel htmlFor="name" value="Nama Bukan Kayu" />
              <TextInput
                id="name"
                type="text"
