...
export default function Edit({ auth, data: item, sumberDana }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    patch(route('reboisasi-ps.update', item.id));
  };
  ...
  return (
    ...
    >
      <Head title="Edit Reboisasi Area PS" />

      <div className="max-w-4xl mx-auto">
        <Link
          href={route('reboisasi-ps.index')}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-8 border-b border-gray-100 bg-gray-50/50">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Perbarui informasi laporan reboisasi area perhutanan sosial di bawah ini.
            </p>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div>
                  ...
                  <Select
                    ...
                    value={regencies.find(r => r.value == data.regency_id) || (item.regency_rel ? { value: item.regency_id, label: formatLabel(item.regency_rel.name) } : null)}
                    onChange={(opt) => {
                      setData((prev) => ({
                        ...prev,
                ...
                <div>
                  ...
                  <Select
                    ...
                    value={districts.find(d => d.value == data.district_id) || (item.district_rel ? { value: item.district_id, label: formatLabel(item.district_rel.name) } : null)}
                    onChange={(opt) => {
                      setData((prev) => ({
                        ...prev,
                ...
                <div>
                  ...
                  <Select
                    ...
                    value={villages.find(v => v.value == data.village_id) || (item.village_rel ? { value: item.village_id, label: formatLabel(item.village_rel.name) } : (item.village_name ? { value: item.village_id, label: item.village_name } : null))}
