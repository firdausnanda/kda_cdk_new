...
export default function Index({ auth, datas, stats, filters, availableYears, sumberDana }) {
  ...
  const handleBulkAction = (action) => {
    ...
    switch (action) {
      case 'delete':
        ...
        routeName = 'reboisasi-ps.bulk-delete';
        color = '#d33';
        break;
      case 'submit':
        title = 'Ajukan Data Terpilih?';
        confirmText = 'Ya, Ajukan!';
        routeName = 'reboisasi-ps.bulk-submit';
        color = '#15803d';
        break;
      case 'approve':
        title = 'Setujui Data Terpilih?';
        confirmText = 'Ya, Setujui!';
        routeName = 'reboisasi-ps.bulk-approve';
        color = '#15803d';
        break;
      default:
        return;
  ...
  const handleYearChange = (year) => {
    ...
    setIsLoading(true);
    router.get(route('reboisasi-ps.index'), {
      year,
      search: searchTerm,
      sort: filters.sort,
  ...
  const handleDelete = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.delete(route('reboisasi-ps.destroy', id), {
          preserveScroll: true,
          onSuccess: () => {
            setIsLoading(false);
            MySwal.fire({
  ...
  const handleSubmit = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('reboisasi-ps.submit', id), {}, {
          preserveScroll: true,
          onFinish: () => setIsLoading(false)
        });
      }
  ...
  const handleVerify = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('reboisasi-ps.approve', id), {}, {
          preserveScroll: true,
          onFinish: () => setIsLoading(false)
        });
      }
  ...
  const handleReject = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('reboisasi-ps.reject', id), {
        ...
        <div className={`space-y-6 transition-all duration-700 ease-in-out ${isLoading ? 'opacity-30 blur-md grayscale-[0.5] pointer-events-none' : 'opacity-100 blur-0'}`}>
          ...
          <div className="bg-gradient-to-r from-primary-800 to-primary-600 rounded-2xl p-8 text-white shadow-lg relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="text-2xl font-bold font-display">Data Reboisasi Area Perhutanan Sosial</h3>
                <p className="mt-1 text-primary-100 opacity-90 max-w-xl text-sm">
                  Kelola dan pantau capaian target tahunan reboisasi area perhutanan sosial di wilayah CDK Trenggalek.
                </p>
              </div>
              {canCreate && (
                <div className="flex gap-2">
                  <button onClick={() => window.location.href = route('reboisasi-ps.export', { year: filters.year })} className="flex items-center gap-2 px-4 py-2.5 bg-primary-700 text-primary-100 rounded-xl font-bold text-sm shadow-sm hover:bg-primary-800 transition-colors border border-primary-600/50">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                    Export
                  </button>
                  <button onClick={() => setShowImportModal(true)} className="flex items-center gap-2 px-4 py-2.5 bg-primary-700 text-primary-100 rounded-xl font-bold text-sm shadow-sm hover:bg-primary-800 transition-colors border border-primary-600/50">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                    Import
                  </button>
                  <Link href={route('reboisasi-ps.create')} className="shrink-0">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          {((canEdit && (item.status === 'draft' || item.status === 'rejected')) || isAdmin) && (
                            <>
                              <Link
                                href={route('reboisasi-ps.edit', item.id)}
      ...
      <Modal show={showImportModal} onClose={() => setShowImportModal(false)}>
        <form onSubmit={(e) => { e.preventDefault(); if (!importFile) return; const formData = new FormData(); formData.append('file', importFile); setLoadingText('Mengimport Data...'); setIsLoading(true); setShowImportModal(false); router.post(route('reboisasi-ps.import'), formData, { forceFormData: true, preserveScroll: true, onFinish: () => { setIsLoading(false); setImportFile(null); }, onError: () => setIsLoading(false) }); }} className="p-0 overflow-hidden">
          ...
          <div className="p-6 space-y-8">
            <div className="flex gap-4 items-start"><div className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-sm">1</div><div className="flex-1"><h3 className="text-sm font-bold text-gray-900 mb-1">Unduh Template</h3><p className="text-xs text-gray-500 mb-3">Gunakan template yang telah disediakan.</p><button type="button" onClick={() => window.location.href = route('reboisasi-ps.template')} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 text-xs font-bold rounded-lg border border-blue-100 hover:bg-blue-100"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>Download Template</button></div></div>
            <div className="border-t border-gray-100"></div>
            <div className="flex gap-4 items-start"><div className="flex-shrink-0 w-8 h-8 rounded-full bg-primary-100 text-primary-600 flex items-center justify-center font-bold text-sm">2</div><div className="flex-1"><h3 className="text-sm font-bold text-gray-900 mb-1">Upload Data</h3><p className="text-xs text-gray-500 mb-3">Pilih file Excel/CSV.</p><div className={`relative border-2 border-dashed rounded-xl p-6 text-center cursor-pointer ${importFile ? 'border-primary-500 bg-primary-50/50' : 'border-gray-200 hover:border-primary-300'}`}><input type="file" accept=".xlsx,.xls,.csv" onChange={(e) => setImportFile(e.target.files[0])} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" /><div className="space-y-2 pointer-events-none"><div className={`mx-auto h-12 w-12 rounded-full flex items-center justify-center ${importFile ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-400'}`}>{importFile ? <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>}</div>{importFile ? <div><p className="text-sm font-bold text-primary-800">{importFile.name}</p><p className="text-xs text-primary-600 mt-1">{(importFile.size / 1024).toFixed(1)} KB</p></div> : <p className="text-sm font-medium text-gray-500">Klik untuk pilih file</p>}</div></div></div></div>
          </div>
