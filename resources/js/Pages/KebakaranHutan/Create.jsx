...
export default function Create({ auth, pengelolaWisata }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    post(route('kebakaran-hutan.store'));
  };
  ...
  const selectStyles = {
    control: (base, state) => ({
      ...
      '&:hover': {
        borderColor: state.isDisabled ? '#f3f4f6' : '#dc2626',
      },
    ...
    dropdownIndicator: (base, state) => ({
      ...
      '&:hover': {
        color: state.isDisabled ? '#d1d5db' : '#dc2626',
      }
    }),
  ...
  return (
    ...
    >
      <Head title="Input Kebakaran Hutan" />

      <div className="max-w-4xl mx-auto">
        <Link
          href={route('kebakaran-hutan.index')}
          className="inline-flex items-center gap-2 text-sm font-semibold text-red-600 hover:text-red-800 transition-colors mb-6 group"
        >
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-8 border-b border-gray-100 bg-gray-50/50">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Lengkapi informasi di bawah ini untuk menambahkan data kebakaran hutan baru.
            </p>
          </div>
            ...
            <form onSubmit={submit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <div className="md:col-span-2">
                  <h4 className="text-xs font-bold text-red-600 uppercase tracking-widest mb-4 border-b border-red-100 pb-2">Informasi Waktu & Lokasi</h4>
                </div>
                ...
                <div>
                  ...
                  <select
                    id="month"
                    className="w-full bg-gray-50 border-gray-200 focus:bg-white focus:border-red-500 focus:ring-red-500 rounded-xl shadow-sm transition-all duration-200 py-[11px] text-sm disabled:bg-gray-100/50 disabled:text-gray-400 disabled:border-gray-100 disabled:cursor-not-allowed"
                    value={data.month}
                ...
                <div className="md:col-span-2 mt-4">
                  <h4 className="text-xs font-bold text-red-600 uppercase tracking-widest mb-4 border-b border-red-100 pb-2">Detail Kejadian</h4>
                </div>

                <div>
                  <InputLabel htmlFor="id_pengelola_wisata" value="Pengelola Hutan" className="text-gray-700 font-bold mb-2" />
                  <Select
                ...
                <div>
                  ...
                  <Select
                    ...
                    styles={selectStyles}
                    isClearable
                    value={data.area_function ? { value: data.area_function, label: data.area_function } : null}
                  />
                  <InputError message={errors.area_function} className="mt-2" />
                </div>

                <div>
                  <InputLabel htmlFor="number_of_fires" value="Jumlah Kejadian" className="text-gray-700 font-bold mb-2" />
                  <div className="relative">
                    <TextInput
                      id="number_of_fires"
                      type="number"
                      className="w-full pr-12"
                      value={data.number_of_fires}
                      onChange={(e) => setData('number_of_fires', e.target.value)}
                      required
                    />
                    <div className="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-gray-400 text-xs font-bold">Kali</div>
                  </div>
                  <InputError message={errors.number_of_fires} className="mt-2" />
                </div>

                <div>
                  <InputLabel htmlFor="fire_area" value="Luas Kebakaran" className="text-gray-700 font-bold mb-2" />
                  <div className="relative">
                    <TextInput
                      id="fire_area"
                      type="text"
                      ...
                      onChange={(e) => setData('fire_area', e.target.value)}
                      required
                    />
                  ...
                  <InputError message={errors.fire_area} className="mt-2" />
                </div>
              ...
              <div className="pt-6 border-t border-gray-100 flex items-center justify-end gap-4">
                ...
                <PrimaryButton
                  className="px-8 py-3 bg-gradient-to-r from-red-700 to-orange-800 hover:from-red-600 hover:to-orange-700 text-white rounded-xl font-bold shadow-lg shadow-red-900/20 transition-all transform active:scale-95"
                  loading={processing}
                >
