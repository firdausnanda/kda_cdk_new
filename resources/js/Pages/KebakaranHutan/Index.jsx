...
export default function Index({ auth, datas, stats, filters, availableYears }) {
  ...
  const handleYearChange = (year) => {
    ...
    setIsLoading(true);
    router.get(route('kebakaran-hutan.index'), {
      year,
      search: searchTerm,
      sort: filters.sort,
              ...
              <table className="w-full text-left text-sm text-gray-500 min-w-[1000px]">
                <thead className="bg-gray-50/50 text-gray-700 uppercase tracking-wider text-[11px] font-bold">
                  <tr>
                    ...
                    <th
                      className="px-6 py-4 cursor-pointer hover:bg-gray-100 transition-colors group"
                      onClick={() => handleSort('area_function')}
                    >
                      <div className="flex items-center gap-1">
                        Fungsi Kawasan
                        <SortIcon field="area_function" />
                      </div>
                    </th>
                    <th
                      className="px-6 py-4 text-center cursor-pointer hover:bg-gray-100 transition-colors group"
                      onClick={() => handleSort('number_of_fires')}
                    >
                      <div className="flex items-center justify-center gap-1">
                        Jml Kejadian
                        <SortIcon field="number_of_fires" />
                      </div>
                    </th>
                    <th
                      className="px-6 py-4 text-center cursor-pointer hover:bg-gray-100 transition-colors group"
                      onClick={() => handleSort('fire_area')}
                    >
                      <div className="flex items-center justify-center gap-1">
                        Luas (Ha)
                        <SortIcon field="fire_area" />
                      </div>
                    </th>
                    <th
                      className="px-6 py-4 text-center cursor-pointer hover:bg-gray-100 transition-colors group"
                ...
                <tbody className="divide-y divide-gray-100">
                  {datas.data.map((item) => (
                    <tr key={item.id} className={`hover:bg-red-50/30 transition-colors group ${selectedIds.includes(item.id) ? 'bg-red-50/50' : ''}`}>
