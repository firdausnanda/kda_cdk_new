...
export default function Index({ auth, datas, stats, filters, availableYears, sumberDana }) {
  ...
  const handleYearChange = (year) => {
    ...
    setIsLoading(true);
    router.get(route('penghijauan-lingkungan.index'), { year, search: searchTerm }, {
      preserveState: true,
      replace: true,
  ...
  const handleSearch = useCallback(
    debounce((value) => {
      router.get(
        route('penghijauan-lingkungan.index'),
  ...
  const handleDelete = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.delete(route('penghijauan-lingkungan.destroy', id), {
          preserveScroll: true,
          onSuccess: () => {
            setIsLoading(false);
            MySwal.fire({
  ...
  const handleSubmit = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('penghijauan-lingkungan.submit', id), {}, {
          preserveScroll: true,
          onFinish: () => setIsLoading(false)
        });
      }
  ...
  const handleVerify = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('penghijauan-lingkungan.approve', id), {}, {
          preserveScroll: true,
          onFinish: () => setIsLoading(false)
        });
      }
  ...
  const handleReject = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('penghijauan-lingkungan.reject', id), {
  ...
  return (
    <>
      <AuthenticatedLayout
        user={auth.user}
        header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Penghijauan Lingkungan</h2>}
      >
        <Head title="Penghijauan Lingkungan" />
        ...
        <div className={`space-y-6 transition-all duration-700 ease-in-out ${isLoading ? 'opacity-30 blur-md grayscale-[0.5] pointer-events-none' : 'opacity-100 blur-0'}`}>
          ...
          <div className="bg-gradient-to-r from-primary-800 to-primary-600 rounded-2xl p-8 text-white shadow-lg relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="text-2xl font-bold font-display">Data Penghijauan Lingkungan (Luar Kawasan)</h3>
                <p className="mt-1 text-primary-100 opacity-90 max-w-xl text-sm">
                  Kelola dan pantau capaian target tahunan penghijauan lingkungan di wilayah CDK Trenggalek.
                </p>
              </div>
              {canCreate && (
                <div className="flex gap-2">
                  ...
                  <Link href={route('penghijauan-lingkungan.create')} className="shrink-0">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          {((canEdit && (item.status === 'draft' || item.status === 'rejected')) || isAdmin) && (
                            <>
                              <Link
                                href={route('penghijauan-lingkungan.edit', item.id)}
      ...
      <Modal show={showImportModal} onClose={() => setShowImportModal(false)}>
        <form onSubmit={(e) => { e.preventDefault(); if (!importFile) return; const formData = new FormData(); formData.append('file', importFile); setLoadingText('Mengimport Data...'); setIsLoading(true); setShowImportModal(false); router.post(route('penghijauan-lingkungan.import'), formData, { forceFormData: true, preserveScroll: true, onFinish: () => { setIsLoading(false); setImportFile(null); }, onError: () => setIsLoading(false) }); }} className="p-0 overflow-hidden">
          <div className="p-6 bg-slate-50 border-b border-gray-100 flex items-center justify-between">
            ...
            <button type="button" onClick={() => setShowImportModal(false)} className="text-gray-400 hover:text-gray-600"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
          </div>
          <div className="p-6 space-y-8">
            <div className="flex gap-4 items-start">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mb-3">Gunakan template yang telah disediakan untuk memastikan format data sesuai.</p>
                <button type="button" onClick={() => window.location.href = route('penghijauan-lingkungan.template')} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 text-xs font-bold rounded-lg border border-blue-100 hover:bg-blue-100"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>Download Template Excel</button>
              </div>
            </div>
            ...
            <div className="flex gap-4 items-start">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mb-3">Pilih file yang telah diisi sesuai template (.xlsx, .xls, .csv).</p>
                <div className={`relative border-2 border-dashed rounded-xl p-6 text-center cursor-pointer ${importFile ? 'border-primary-500 bg-primary-50/50' : 'border-gray-200 hover:border-primary-300'}`}>
                  ...
                  <div className="space-y-2 pointer-events-none">
                    <div className={`mx-auto h-12 w-12 rounded-full flex items-center justify-center ${importFile ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-400'}`}>
          ...
          <div className="p-6 bg-gray-50 flex justify-end gap-3 border-t border-gray-100">
            ...
            <button type="submit" className="px-6 py-2 bg-primary-600 text-white font-bold rounded-xl hover:bg-primary-700 disabled:opacity-50" disabled={!importFile}>Proses Import</button>
          </div>
        </form>
      </Modal>
    </>
