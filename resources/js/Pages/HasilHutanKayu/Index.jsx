...
export default function Index({ auth, datas, forest_type, filters, stats, available_years }) {
  ...
  const handleYearChange = (e) => {
    ...
    router.get(route('hasil-hutan-kayu.index'), { forest_type, year: selectedYear }, {
      preserveState: true,
      preserveScroll: true,
    });
  };
  ...
  const handleSearch = useCallback(
    debounce((value) => {
      router.get(
        ...
        { forest_type, year: filters.year, search: value },
        {
          preserveState: true,
  ...
  const handleDelete = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.delete(route('hasil-hutan-kayu.destroy', id), {
          preserveScroll: true,
          onSuccess: () => {
            setIsLoading(false);
            MySwal.fire({
  ...
  const handleSubmit = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('hasil-hutan-kayu.submit', id), {}, {
          preserveScroll: true,
          onFinish: () => setIsLoading(false)
        });
      }
  ...
  const handleVerify = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('hasil-hutan-kayu.approve', id), {}, {
          preserveScroll: true,
          onFinish: () => setIsLoading(false)
        });
      }
  ...
  const handleReject = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setIsLoading(true);
        router.post(route('hasil-hutan-kayu.reject', id), {
  ...
  return (
    <AuthenticatedLayout
      user={auth.user}
      header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Hasil Hutan Kayu - {forest_type}</h2>}
    >
      <Head title={`Hasil Hutan Kayu - ${forest_type}`} />
      ...
      <div className={`space-y-6 transition-all duration-700 ease-in-out ${isLoading ? 'opacity-30 blur-md grayscale-[0.5] pointer-events-none' : 'opacity-100 blur-0'}`}>
        ...
        <div className={`bg-gradient-to-r ${getHeaderColor()} rounded-2xl p-8 text-white shadow-lg relative overflow-hidden`}>
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h3 className="text-2xl font-bold font-display">Data Hasil Hutan Kayu</h3>
              <p className="mt-1 text-emerald-100 opacity-90 max-w-xl text-sm">
                Kelola data hasil hutan kayu untuk kategori {forest_type}.
              </p>
            </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-emerald-600 mt-1">{formatNumber(stats.total_volume)} <span className="text-xs font-normal text-gray-400">m³</span></p>
              </div>
